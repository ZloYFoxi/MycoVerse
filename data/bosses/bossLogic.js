Game.bosses = (function () {
    ...
    instance.getBoss = function (id) {
        return Game.bossData.entries[id] || null;
    };

    instance.getBossForPlanet = function (planetId) {
        return this.getBoss(Game.bossData.planetBosses[planetId]);
    };

    instance.isDefeated = function (bossId) {
        return !!this.defeated[bossId];
    };

    instance.isPlanetBossDefeated = function (planetId) {
        var boss = this.getBossForPlanet(planetId);
        return !boss || this.isDefeated(boss.id);
    };
    ...
    instance.autoSelectTeam = function () {
        ...
        if (Game.miners && Game.miners.getEntriesSorted) {
            ...
            for (var i = 0; i < entries.length; i++) {
                if (entries[i].owned <= 0) continue;
                candidates.push({ id: entries[i].id, power: this.getMinerCombatPower(entries[i].id) });
            }
        }
    ...
    instance.toggleTeamMember = function (minerId) {
        ...
        if (this.selectedTeam.length >= Game.bossData.teamSize) {
            Game.notifyInfo("Team full", "A boss squad may contain up to " + Game.bossData.teamSize + " species.");
            return false;
        }
        this.selectedTeam.push(minerId);
        return true;
    };

    instance.getMinerCombatPower = function (minerId) {
        var entry = Game.miners.getEntry(minerId);
        if (!entry || entry.owned <= 0) return 0;
        ...
        return base * (1 + mutation / 100);
    };

    instance.getTeamPower = function () {
        return this.getBattleTeamPower(this.selectedTeam);
    };

    instance.getPhase = function (boss, health) {
        var ratio = Math.max(0, health) / boss.maxHealth;
        var phase = { name: "Opening Assault", damageMultiplier: 1 };
        for (var i = 0; i < boss.phases.length; i++) {
            if (ratio <= boss.phases[i].at) phase = boss.phases[i];
        }
        return phase;
    };
    ...
    instance.canStart = function (bossId) {
        ...
        if (!Game.planets.isUnlocked(boss.planetId)) return false;
        return this.selectedTeam.length > 0 && this.getTeamPower() > 0;
    };

    instance.startBattle = function (bossId) {
        if (!this.canStart(bossId)) {
            Game.notifyInfo("Battle unavailable", "Unlock the planet and assemble a living team before challenging this boss.");
            return false;
        }
        var boss = this.getBoss(bossId);
        var t = now();
        this.activeBattle = {
            ...
            teamPower: this.getTeamPower()
        };
        Game.notifySuccess("Boss battle started", "Your squad has engaged " + boss.name + ".");
        return true;
    };
    ...
    instance.cancelBattle = function () {
        ...
        this.activeBattle = null;
        Game.notifyInfo("Squad retreated", "The colony withdrew before the battle was lost.");
        return true;
    };
    ...
    instance.getActiveSnapshot = function () {
        ...
        return {
            ...
            phase: phase,
            teamPower: num(this.activeBattle.teamPower, this.getBattleTeamPower(this.activeBattle.team))
        };
    };
    ...
    instance.applyRewards = function (boss) {
        ...
        if (Game.account) {
            Game.account.add("mycoCoins", reward.mycoCoins || 0);
            Game.account.add("bloomTokens", reward.bloomTokens || 0);
        }
        ...
        if (reward.nextPlanetId && Game.planets && Game.planetData.planets[reward.nextPlanetId]) {
            Game.planets.unlocked[reward.nextPlanetId] = true;
        }
    };
    ...
    instance.completeVictory = function () {
        ...
        this.activeBattle = null;
        Game.notifySuccess("Planet boss defeated", boss.name + " has fallen. The next league and exclusive rewards are now yours.");
    };
    ...
    instance.completeDefeat = function () {
        ...
        this.history = this.history.slice(0, 20);
        this.activeBattle = null;
        Game.notifyInfo("Battle lost", boss.name + " survived the assault. Strengthen your squad and try again.");
    };
    ...
    instance.update = function () {
        ...
        var t = now();
        var effectiveNow = Math.min(t, this.activeBattle.endsAt);
        var elapsed = Math.max(0, (effectiveNow - this.activeBattle.lastTickAt) / 1000);
        if (elapsed > 0) {
            var remaining = elapsed;
            var teamPower = num(this.activeBattle.teamPower, this.getBattleTeamPower(this.activeBattle.team));
            this.activeBattle.teamPower = teamPower;
            while (remaining > 0 && this.activeBattle.health > 0) {
        ...
        else if (t >= this.activeBattle.endsAt) this.completeDefeat();
    };
