Game.inventoryUI = (function () {
    ...
    instance.render=function(){
        ...
        for(var i=0;i<items.length;i++){
            ...
            if(x.type==="miner") {
                var minerEntry = Game.miners.getEntry ? Game.miners.getEntry(x.id) : null;
                if (!minerEntry) {
                    var entries = Game.miners.getEntriesSorted();
                    for (var mi=0; mi<entries.length; mi++) if (entries[mi].id===x.id) { minerEntry=entries[mi]; break; }
                }
                html.push(Game.minerCardUI.ownedCard(x.id, minerEntry || {definition:Game.minerData[x.id],owned:x.owned,level:x.level}, {}));
            } else {
