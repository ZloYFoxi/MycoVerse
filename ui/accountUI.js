Game.accountUI = (function () {
    ...
    instance.render=function(){
        ...
        $("#accountLevelText").text("Commander Level "+lvl.level+" • "+fmt(lvl.current)+" / "+fmt(lvl.required));$("#accountLevelProgress").css("width",lvl.percent+"%");$("#accountBackendStatus").text(Game.backend.getStatusLabel());$("#accountNameInput").val(s.name);
        ...
        var r=Game.account.getDailyReward(),can=Game.account.canClaimDaily();$("#dailyRewardText").text("+"+r.mycoCoins+" MycoCoins and +"+r.bloomTokens+" Bloom Token");$("#claimDailyReward").prop("disabled",!can);$("#dailyRewardTimer").text(can?"Reward ready now.":"Next reward in "+time(Game.account.getDailyRemaining()));
    };
